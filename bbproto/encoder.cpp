...
namespace asn1 {
	...
	inline size_t encode_bbcmd (Allocator * allocator, char * buff, size_t buff_ln, unsigned wm)
	{
		Command_t command;
		memset(&command, 0, sizeof(Command));
...
namespace bb {
	...
	size_t encode_bbcmd (char * buff, size_t buff_ln, unsigned wm)
	{
		asn1::Header & hdr = asn1::encode_header(buff, buff_ln);
		asn1::Asn1StackAllocator a;
		const size_t n = asn1::encode_bbcmd(&a, buff + sizeof(asn1::Header), buff_ln - sizeof(asn1::Header), wm);
		hdr.m_len = n;
		return n + sizeof(asn1::Header);
	}
}
