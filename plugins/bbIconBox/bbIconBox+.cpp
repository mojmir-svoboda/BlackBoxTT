...
const char szVersion     [] = "bbIconBox+ 7.11";
...
#define NOT_XOBLITE BBVERSION_WIN == BBVersion || BBVERSION_LEAN == BBVersion
...
void icon_box::paint_text(HDC hdc, RECT *r, StyleItem *pSI, StyleItem *pTC, bool centerd, const char *text)
{
     ...
     {
         RECT s = {0,0,0,0};
         BB_DrawText(hdc, text, -1, &s, DT_LEFT|DT_EXPANDTABS|DT_CALCRECT, pSI);
     ...
     BB_DrawText(hdc, text, -1, r, just | DT_VCENTER | DT_SINGLELINE | DT_EXPANDTABS, pSI);
...
int beginPluginEx(HINSTANCE hPluginInstance, HWND hSlit)
{
    ...
    if (0 == memicmp(bbv, "bbLean|bb4win_mod", 18)) BBVersion = BBVERSION_WIN;
    else
