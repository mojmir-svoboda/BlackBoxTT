	...
	switch (msg)
	{
		...
		case WM_PAINT:
			...
			if (style_draw_begin(c, d))
			{
				...
				int style_index = (details->hilite || details->is_on) ? get_pressedstyle_index(c->windowptr->style):c->windowptr->style;
				if (details->hilite || details->is_on)
				{
					if (c->windowptr->bstyleptr->has_custom_style)
					{
						style_draw_box(
							d.rect,
							d.buffer,
							c->windowptr->bstyleptr->styleptr,
					...
					else
					{
						style_draw_box(
							d.rect,
							d.buffer,
							(c->windowptr->bstyleptr->style == STYLETYPE_DEFAULT)?style_index:c->windowptr->bstyleptr->style,
							c->windowptr->is_bordered
						);
					}
				}
					...
					if (details->hilite || details->is_on)
					{
						...
						if (c->windowptr->bstyleptr->has_custom_style)
						{
							style_draw_text(
								...
								details->caption,
								details->settings,
								c->windowptr->bstyleptr->use_custom_font ? c->windowptr->bstyleptr->font :c->windowptr->use_custom_font?c->windowptr->font:NULL
							);
						}
						else
						{
