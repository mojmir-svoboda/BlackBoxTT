...
namespace cedar {
  ...
  public:
    ...
    // interfance
    template <typename T>
    T exactMatchSearch (const TCHAR* key) const
    { return exactMatchSearch <T> (key, std::strlen (key)); }
    template <typename T>
    T exactMatchSearch (const TCHAR* key, size_t len, size_t from = 0) const {
    ...
    template <typename T>
    size_t commonPrefixSearch (const TCHAR* key, T* result, size_t result_len) const
    { return commonPrefixSearch (key, result, result_len, std::strlen (key)); }
    template <typename T>
    size_t commonPrefixSearch (const TCHAR* key, T* result, size_t result_len, size_t len, size_t from = 0) const {
      size_t num = 0;
    ...
    template <typename T>
    size_t commonPrefixPredict (const TCHAR* key, T* result, size_t result_len)
    { return commonPrefixPredict (key, result, result_len, _tcslen (key)); }
    template <typename T>
    size_t commonPrefixPredict (const TCHAR* key, T* result, size_t result_len, size_t len, size_t from = 0) {
    ...
    void suffix (TCHAR* key, size_t len, size_t to) const {
      key[len] = '\0';
      while (len--) {
        const int from = _array[to].check;
        key[len] = static_cast <TCHAR> (_array[from].base () ^ static_cast <int> (to));
        to = static_cast <size_t> (from);
      }
    }
    value_type traverse (const TCHAR* key, size_t& from, size_t& pos) const
    { return traverse (key, from, pos, std::strlen (key)); }
    value_type traverse (const TCHAR* key, size_t& from, size_t& pos, size_t len) const {
      union { int i; value_type x; } b;
      b.i = _find (key, from, pos, len);
      return b.x;
    }
    struct empty_callback { void operator () (const int, const int) {} }; // dummy empty function
    value_type& update (const TCHAR* key)
    { return update (key, std::strlen (key)); }
    value_type& update (const TCHAR* key, size_t len, value_type val = value_type (0))
    { size_t from (0), pos (0); return update (key, from, pos, len, val); }
    value_type& update (const TCHAR* key, size_t& from, size_t& pos, size_t len, value_type val = value_type (0))
    { empty_callback cf; return update (key, from, pos, len, val, cf); }
    template <typename T>
    value_type& update (const TCHAR* key, size_t& from, size_t& pos, size_t len, value_type val, T& cf) {
      if (! len && ! from)
        _err (__FILE__, __LINE__, "failed to insert zero-length key\n");
#ifndef USE_FAST_LOAD
      if (! _ninfo || ! _block) restore ();
#endif
      for (const TCHAR * const key_ = reinterpret_cast <const TCHAR*> (key);
           pos < len; ++pos) {
#ifdef USE_REDUCED_TRIE
    ...
    int erase (const TCHAR* key) { return erase (key, std::strlen (key)); }
    int erase (const TCHAR* key, size_t len, size_t from = 0) {
      size_t pos = 0;
    ...
    int build (size_t num, const TCHAR** key, const size_t* len = 0, const value_type* val = 0) {
    ...
    int save (const TCHAR* fn, const TCHAR* mode = TEXT("wb")) const {
      // _test ();
      FILE* fp = _tfopen(fn, mode);
      if (! fp) return -1;
    ...
    int open (const TCHAR* fn, const TCHAR* mode = TEXT("rb"),
  ...
  private:
    ...
    int _find (const TCHAR* key, size_t& from, size_t& pos, const size_t len) const {
