...
size_t const SHA1_DIGEST_SIZE=20;
...
/* Compute SHA1 message digest for bytes read from STREAM.  The
   ...
   beginning at RESBLOCK.  */
inline int sha1_stream (FILE *stream, void *resblock)
{
  struct sha1_ctx ctx;
  size_t sum;

  char *buffer = static_cast<char *>(malloc (BLOCKSIZE + 72));
  if (!buffer)
    return 1;
...
/* Compute SHA1 message digest for LEN bytes beginning at BUFFER.  The
   ...
   output yields to the wanted ASCII representation of the message
   digest.  */
inline void * sha1_buffer (const char *buffer, size_t len, void *resblock)
{
  struct sha1_ctx ctx;
...
inline void sha1_process_bytes (const void *buffer, size_t len, struct sha1_ctx *ctx)
{
  ...
  if (ctx->buflen != 0)
    {
      ...
      ctx->buflen += static_cast<uint32_t>(add);
  ...
  if (len > 0)
    {
      ...
      ctx->buflen = static_cast<uint32_t>(left_over);
    }
}
...
{
  const uint32_t *words = static_cast<uint32_t const *>(buffer);
  ...
  ctx->total[0] += static_cast<uint32_t>(len);
  if (ctx->total[0] < len)
    ++ctx->total[1];
