...
SET_OF_decode_ber(Allocator * allocator, asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
	...
	microphase2:
		 ...
		 */
		rval = elm->type->ber_decoder(allocator, opt_codec_ctx,
		...
		case RC_OK:
			{
				...
				if(ASN_SET_ADD(allocator, list, ctx->ptr) != 0)
					RETURN(RC_FAIL);
				else
					ctx->ptr = 0;
			}
		...
		case RC_FAIL: /* Fatal error */
			ASN_STRUCT_FREE(allocator, *elm->type, ctx->ptr);
			ctx->ptr = 0;
			RETURN(RC_FAIL);
		} /* switch(rval) */
...
static int _el_addbytes(Allocator * allocator, const void *buffer, size_t size, void *el_buf_ptr) {
...
SET_OF_encode_der(Allocator * allocator, asn_TYPE_descriptor_t *td, void *ptr,
	...
	for(edx = 0; edx < list->count; edx++) {
		...
		erval = der_encoder(allocator, elm_type, memb_ptr, 0, elm->tag, 0, 0);
		if(erval.encoded == -1)
			return erval;
	 ...
	 */
	encoding_size = der_write_tags(allocator, td, computed_size, tag_mode, 1, tag,
		cb, app_key);
	if(encoding_size == -1) {
		erval.encoded = -1;
	 ...
	 * encoded elements.
	 */
	encoded_els = (struct _el_buffer *)CXX_ALLOC_WRAP MALLOC(
	...
	for(edx = 0; edx < list->count; edx++) {
		 ...
		 */
		encoded_el->buf = (uint8_t *)CXX_ALLOC_WRAP MALLOC(max_encoded_len);
		if(encoded_el->buf) {
			encoded_el->length = 0;
			encoded_el->size = max_encoded_len;
		} else {
			for(edx--; edx >= 0; edx--)
				CXX_ALLOC_WRAP FREEMEM(encoded_els[edx].buf);
			CXX_ALLOC_WRAP FREEMEM(encoded_els);
			erval.encoded = -1;
			erval.failed_type = td;
			erval.structure_ptr = ptr;
			return erval;
		 ...
		 */
		erval = der_encoder(allocator, elm_type, memb_ptr, 0, elm->tag,
	...
	for(edx = 0; edx < eels_count; edx++) {
		...
		if(ret == 0
		&& cb(allocator, encoded_el->buf, encoded_el->length, app_key) < 0)
			ret = -1;
		CXX_ALLOC_WRAP FREEMEM(encoded_el->buf);
	}
	CXX_ALLOC_WRAP FREEMEM(encoded_els);
...
SET_OF_decode_xer(Allocator * allocator, asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
	...
	for(; ctx->phase <= 2;) {
		...
		if(ctx->phase == 2) {
			...
			tmprval = element->type->xer_decoder(allocator, opt_codec_ctx,
...
SET_OF_encode_xer(Allocator * allocator, asn_TYPE_descriptor_t *td, void *sptr,
	...
	if(xcan) {
		encs = (xer_tmp_enc_t *)CXX_ALLOC_WRAP MALLOC(list->count * sizeof(encs[0]));
	...
	for(i = 0; i < list->count; i++) {
		...
		if(tmper.encoded == 0 && specs->as_XMLValueList) {
			...
			_ASN_CALLBACK3(allocator, "<", 1, name, len, "/>", 2);
		}
	...
	if(encs) {
		...
		for(; enc < end; enc++) {
			_ASN_CALLBACK(allocator, enc->buffer, enc->offset);
			CXX_ALLOC_WRAP FREEMEM(enc->buffer);
			enc->buffer = 0;
	...
	if(encs) {
		while(encs_count-- > 0) {
			if(encs[encs_count].buffer)
				CXX_ALLOC_WRAP FREEMEM(encs[encs_count].buffer);
		}
		CXX_ALLOC_WRAP FREEMEM(encs);
	}
	_ASN_ENCODED_OK(er);
}
...
SET_OF_print(Allocator * allocator, asn_TYPE_descriptor_t *td, const void *sptr, int ilevel,
	...
	/* Dump preamble */
	if(cb(allocator, td->name, strlen(td->name), app_key) < 0
	|| cb(allocator, " ::= {", 6, app_key) < 0)
		return -1;
	...
	for(i = 0; i < list->count; i++) {
		...
		ret = elm->type->print_struct(allocator, elm->type, memb_ptr,
			ilevel + 1, cb, app_key);
		if(ret) return ret;
	}
	...
	return (cb(allocator, "}", 1, app_key) < 0) ? -1 : 0;
}
...
SET_OF_free(Allocator * allocator, asn_TYPE_descriptor_t *td, void *ptr, int contents_only) {
	if(td && ptr) {
		...
		asn_set_empty(allocator, list);	/* Remove (list->array) */
...
SET_OF_decode_uper(Allocator * allocator, asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
	...
	do {
		...
		for(i = 0; i < nelems; i++) {
			...
			if(rv.code == RC_OK) {
				if(ASN_SET_ADD(allocator, list, ptr) == 0)
					continue;
			...
			if(ptr) ASN_STRUCT_FREE(allocator, *elm->type, ptr);
			return rv;
		}
